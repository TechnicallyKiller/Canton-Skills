...
description: >-
  ...
  (daml-authorization-patterns), tests (daml-testing), the Ledger API, or
  deployment — defer those to the specific skill.
---
...
designing an app, or calling the Ledger API — the other skills assume it.
...
4. **Structural authorization.** Who can do what is declared in Daml at compile
   time (signatories / observers / controllers) and enforced by the protocol — not
   by `require`/`msg.sender` checks in application code.

## Apply this before you build

When a task is Canton/Daml-shaped, reason in this order:
